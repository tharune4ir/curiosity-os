...
export default function AINexus() {
    ...
    const mdDirectory = path.join(process.cwd(), "content", "9_ai");
    ...
    if (!fs.existsSync(mdDirectory)) {
        return <div className="p-20 text-white flex items-center justify-center min-h-screen bg-[#020617]">
            <div className="text-xl font-mono animate-pulse">Initializing AI Nexus...</div>
        </div>;
    }
    ...
    return (
        <main className="relative flex h-screen w-full bg-[#020617] overflow-hidden">
            <UniverseGraph graphData={graphData} storageNamespace="ai" />
        </main>
    );
}
