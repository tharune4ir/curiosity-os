...
export default function DigitalOS() {
    // 1. Path to our markdown files
    const mdDirectory = path.join(process.cwd(), "content", "8_digital");

    // Ensure directory exists to avoid build errors if run before generation
    if (!fs.existsSync(mdDirectory)) {
        return <div className="p-20 text-white flex items-center justify-center min-h-screen bg-[#020617]">
            <div className="text-xl font-mono animate-pulse">Initializing Digital Universe...</div>
        </div>;
    }
    ...
    return (
        <main className="relative flex h-screen w-full bg-[#020617] overflow-hidden">
            <UniverseGraph graphData={graphData} storageNamespace="digital" />
        </main>
    );
}
