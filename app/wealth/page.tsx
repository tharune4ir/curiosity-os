...
export default function WealthOS() {
    ...
    const mdDirectory = path.join(process.cwd(), "content", "7_wealth");
    ...
    if (!fs.existsSync(mdDirectory)) {
        return <div className="p-20 text-white flex items-center justify-center min-h-screen bg-[#020617]">
            <div className="text-xl font-mono animate-pulse">Initializing WealthOS Universe...</div>
        </div>;
    }
    ...
    return (
        <main className="relative flex h-screen w-full bg-[#020617] overflow-hidden">
            <UniverseGraph graphData={graphData} storageNamespace="wealth" />
        </main>
    );
}
