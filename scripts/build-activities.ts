...
function buildActivities() {
  ...
  const activities = files.map(file => {
    ...
    const { data, content: body } = matter(content) as any;
  ...
  const index = activities.map(a => ({
    ...
    materials: a.materials.minimal,
    evidence: a.internal.evidence_dimensions_targeted,
    wing: a.internal.backend_primary_wing
  }));
...
function buildCollections(activities: any[]) {
  ...
  const collections = files.map(file => {
    ...
    return { ...data, activities: enriched_activities, body: body.trim() };
  });
...
function buildCoverageMap(activities: any[]) {
  const wings: any = {
  ...
  activities.forEach(a => {
    ...
    if (wings[wing]) {
      wings[wing].count++;
      (a.internal.backend_capability_clusters || []).forEach((cluster: string) => {
        wings[wing].clusters[cluster] = (wings[wing].clusters[cluster] || 0) + 1;
      });
    }

    (a.internal.backend_node_hooks || []).forEach((hook: any) => {
...
console.log("🚀 Building Curiosity OS 2.0 Activity Engine (TypeScript)...");
