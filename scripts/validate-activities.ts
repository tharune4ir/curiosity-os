...
function validateActivity(filePath: string) {
  ...
  if (data.internal) {
    ...
    (data.internal.backend_capability_clusters || []).forEach((cluster: string) => {
      if (!ENUMS.CapabilityCluster.includes(cluster)) {
        logError(filePath, `Invalid cluster: ${cluster}`);
      }
    });

    (data.internal.evidence_dimensions_targeted || []).forEach((dim: string) => {
...
console.log("🔍 Validating Curiosity OS 2.0 Activity Content (TypeScript)...");
